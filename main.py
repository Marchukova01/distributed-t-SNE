...
n_steps = 100  # binary search

early_exaggeration = 12.0
desired_perplexity = 30.0
initial_momentum = 0.5
final_momentum = 0.8
eta = 400 # learning_rate
min_gain = 0.01
...
Y  = np.random.rand(dim_low, n_samples)
Y = Y / np.std(Y[0, :]) * 1e-4

Y = tsne.tsne(X, Y, y, n_samples, m_coord, dim_low, d, max_iter, n_steps, early_exaggeration, desired_perplexity, eta)
graph.graph_fun(Y, y)
